...
@app.route('/send_report_by_login/<organizer_login>', methods=['POST'])
def send_report_by_login(organizer_login):
    organizer = User.query.filter_by(login=organizer_login, role='organizer').first()
    if not organizer:
        return jsonify({'error': 'Organizer not found'}), 404

    matches = Match.query.filter_by(created_by=organizer.id).all()
    if not matches:
        return jsonify({'error': 'No matches found for this organizer'}), 404

    report_lines = []
    for match in matches:
        total_tickets = match.ticket_quantity
        reserved_count = Ticket.query.filter_by(match_id=match.match_id, status='reserved').count()
        sold_sum = reserved_count * float(match.ticket_price)
        remaining = total_tickets - reserved_count
        percent_full = (reserved_count / total_tickets) * 100 if total_tickets else 0

        report_lines.append(
            f"🏟 {match.match_name} ({match.date_time.strftime('%Y-%m-%d')}):\n"
            f" - Продано: {reserved_count} билетов\n"
            f" - Сумма: {sold_sum:.2f} ₽\n"
            f" - Осталось: {remaining}\n"
            f" - Заполнено: {percent_full:.1f}%\n"
        )

    report_body = f"👋 Привет, {organizer.name}!\n\nВот отчет по вашим матчам:\n\n" + "\n".join(report_lines)

    if organizer.email:
        send_email(organizer.email, "📊 Отчет по матчам", report_body)
        return jsonify({'message': f'Отчет отправлен на {organizer.email}'}), 200
    else:
        return jsonify({'message': 'Email не указан у организатора'}), 200
